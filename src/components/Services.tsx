...
const Services = () => {
  ...
  return (
    <section
      id="services"
      className={`py-24 relative overflow-hidden ${theme === 'dark' ? 'bg-dark-card' : 'bg-white'}`}
    >
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {displayedServices.map((service, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`group rounded-2xl p-6 backdrop-blur-sm transition-transform duration-300 hover:scale-105 hover:-translate-y-2 ${
                    theme === 'dark'
    ...
    >
      {displayedServices.map((service, index) => {
        ...
        return (
          <div
            key={index}
            className={`service-card snap-center flex-shrink-0 rounded-2xl p-6 max-w-[350px] w-full mx-3 transition-transform duration-300
              ${theme === 'dark' ? 'bg-dark-bg/50 border border-electric-blue/20' : 'bg-gray-50/80 shadow-lg'}`}
            // Removed hover classes here
          >
                    <div
